...
// Fix: Implement Gemini API calls for script, voiceover, and timing generation.
...
export const generateVoiceover = async (script: string, voice: VoiceOption): Promise<{ audioBlob: Blob, audioUrl: string }> => {
  ...
  // Fix: Cast window to 'any' to access AudioContext/webkitAudioContext, resolving TS error when 'dom' lib is not included.
  ...
  const audioUrl = (window as any).URL.createObjectURL(audioBlob);
