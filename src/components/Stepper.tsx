...
import { UploadIcon, ScriptIcon, MicIcon, PlayIcon, CropIcon } from './icons';
...
const steps = [
  ...
  { id: 4, name: 'Adjust Crops', icon: CropIcon },
  { id: 5, name: 'Preview & Export', icon: PlayIcon },
];
          ...
          <li key={step.name} className={`relative ${stepIdx !== steps.length - 1 ? 'pr-8 sm:pr-20' : ''} flex-1`}>
            {step.id < currentStep ? (
              ...
              <>
                ...
                <span className="absolute top-10 left-1/2 -translate-x-1/2 w-max text-center text-xs text-purple-400 sm:text-sm">{step.name}</span>
              </>
            ) : (
              <>
                <div className="absolute inset-0 flex items-center" aria-hidden="true">
