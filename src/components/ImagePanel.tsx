...
const ImagePanel: React.FC<ImagePanelProps> = ({ panels, onPanelsChange, onNext }) => {
  ...
  useEffect(() => {
    ...
    return () => {
        panels.forEach(panel => URL.revokeObjectURL(panel.preview));
    }
  }, [panels]);

  const handleDragStart = (e: React.DragEvent<HTMLDivElement>, panel: ManhwaPanel) => {
  ...
  const handleDropOnPanel = (e: React.DragEvent<HTMLDivElement>, targetPanel: ManhwaPanel) => {
    e.preventDefault();
